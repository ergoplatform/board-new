...
import org.ergoplatform.board.handlers.{ElectionResources, SwaggerSupport, VoterResources}
...
trait Rest { self: Services with Setup =>
  ...
  lazy val routes = electionRoutes ~ swaggerStaticClientRoute ~ swaggerJsonRoute ~ voterRoutes
}
